...
export function useLocations() {
  ...
  useRealtimeSubscription('locations', (payload: RealtimePayload) => {
  ...
  const fetchLocations = async () => {
    try {
      ...
      const frequent = (data || []).filter(loc => (loc.frequency || 0) > 1)
        .sort((a, b) => (b.frequency || 0) - (a.frequency || 0))
        .slice(0, 5);
      setFrequentLocations(frequent);
    } catch (err) {
  ...
  const updateLocationFrequency = async (location: Location) => {
    try {
      const { error } = await supabase
        .from('locations')
        .update({
          frequency: (location.frequency || 0) + 1
        })
        .eq('id', location.id);

      if (error) throw error;
    } catch (err) {
      console.error('Error updating location frequency:', err);
    }
  };
