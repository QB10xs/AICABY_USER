...
const ServiceStep: React.FC<ServiceStepProps> = ({
  ...
  const selectedVehicle = bookingData.vehicleType || 'Standard';
  const selectedServices = bookingData.additionalServices || [];
  const passengers = bookingData.passengers || 1;

  const handleVehicleSelect = (vehicleId: string) => {
    onUpdate({ vehicleType: vehicleId });
  };

  const toggleService = (serviceId: string) => {
    onUpdate({
      additionalServices: selectedServices.includes(serviceId)
    ...
    <div className="space-y-8">
      ...
      <div className="glass-card p-6">
        ...
        <div className="flex items-center space-x-4">
          <button
            onClick={() => onUpdate({ passengers: Math.max(1, passengers - 1) })}
          ...
          <button
            onClick={() => onUpdate({ passengers: Math.min(6, passengers + 1) })}
            className="w-10 h-10 rounded-full border border-gray-600 flex items-center justify-center hover:bg-white/5"
          >
            +
          </button>
